...
def hsja(model,
    ...
    """
    # Set parameters
    original_label = np.argmax(model(torch.from_numpy(sample).unsqueeze(0).permute(0, 3, 1, 2).float()).detach().numpy())
...
def decision_function(model, images, params):
    ...
    prob = model(torch.from_numpy(images).permute(0, 3, 1, 2).float()).detach().numpy()
